...
Adobe Campaign är en plattform för att utforma flerkanaliga kundupplevelser och en miljö för visuell kampanjsamordning, interaktionshantering i realtid och flerkanalsmarknadsföring.

Adobe Campaign v8 är nästa generationens kampanjverktyg som har byggts för olika marknadsföringskanaler som e-post, push-meddelanden, SMS och direktreklam. Den erbjuder robusta ETL- och datahanteringsfunktioner för att hjälpa till att utforma och strukturera den perfekta kampanjen. Dess orkestreringsmotor möjliggör multitouch-marknadsföring med fokus på batchdrivna resor. Den innehåller även en skalbar realtidsserver för meddelanden som gör det möjligt för marknadsföringsteamen att skicka fördefinierade meddelanden baserat på en omfattande nyttolast från vilket IT-system som helst för kommunikation som lösenordsåterställningar, orderbekräftelser, e-kvitton och mycket annat.

Använd Campaign för att

* **Driv** personalisering och engagemang genom en enda lättillgänglig vy av kunden.
...
Campaign är ursprungligen endast tillgängligt via en omfattande [klientkonsol](#ac-client) och har nu ett nytt webbanvändargränssnitt med förbättrad användarvänlighet, tillgänglighet och en ny design som avsevärt förbättrar användarupplevelsen. Detta moderna användargränssnitt förenklar utformningen och leveransen av marknadsföringskampanjer och ger enhetlighet med andra Adobe-lösningar, inklusive Adobe Experience Platform.

![Skärmbild på startskärmen i Adobe Campaign webbgränssnitt](assets/home.png){zoomable="yes"}

Det nya webbanvändargränssnittet uppfyller i första hand kraven från **företagsanvändare**. Vanliga administrationsåtgärder är inte tillgängliga i den här initiala versionen, men kommer att åtgärdas i efterföljande versioner. Observera att inte alla funktioner eller alternativ som är tillgängliga i klientkonsolen är tillgängliga i det nya användargränssnittet. Nya användningsexempel, alternativ och funktioner kommer att introduceras i framtida versioner.

Om du som administratör eller expertanvändare behöver komma åt Campaign v8-funktioner som inte är tillgängliga i användargränssnittet för Campaign-webben ansluter du till [klientkonsolen](#ac-client).
...
Klientkonsolen är utformad för att administratörer och avancerade användare ska kunna utföra sina uppgifter. Campaign-klientkonsolen är ett webbläsarbaserat program som är installerat på datorn. Det använder API:t för webbtjänster för att ansluta till Campaign-programservern.

Kampanjdata lagras på programservern. Data är tillgängliga både från klientkonsolen och Campaign-webbgränssnittet. Om du till exempel skapar en leveransmall med hjälp av klientkonsolen är den också tillgänglig i webbgränssnittet för Campaign. Om du skapar en e-postleverans i användargränssnittet för Campaign-webben är leveransen tillgänglig från klientkonsolen.

Vissa objekt kan bara skapas och hanteras i klientkonsolen. De här objekten kan vara synliga och användas i gränssnittet för Campaign-webben, men kan inte skapas eller ändras från den här miljön. Alla Campaign-objekt och -komponenter är tillgängliga från Utforskaren, som du kommer åt från den vänstra navigeringen.
...
>[!VIDEO](https://video.tv.adobe.com/v/3453426?captions=swe&quality=12)
-->
...
>[!VIDEO](https://video.tv.adobe.com/v/3448368?captions=swe&quality=12)
