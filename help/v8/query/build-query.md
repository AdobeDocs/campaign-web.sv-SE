...
* **Med filtreringskomponenter** (Anpassat villkor, Välj målgrupp, Fördefinierat filter) kan du skapa egna regler, välja en målgrupp eller ett fördefinierat filter för att förfina din fråga. De läggs till i början av frågan och på prickade övergångar. [Lär dig hur du arbetar med att filtrera komponenter](#filtering)
...
1. Definiera det förväntade värdet i fältet **Värde**. Du kan också använda redigeringsprogrammet för webbuttryck i Campaign för att manuellt definiera ett uttryck med hjälp av fält från databasen och hjälpfunktionerna. Det gör du genom att klicka på knappen **Redigera uttryck** . [Lär dig hur du redigerar uttryck](expression-editor.md)
